...
int somaMatriz(tipoMatriz M, int lin, int col)
{
    int l, c, soma=0;
    for (l = 0; l < lin; l++)
    {
...
void multiplicaMatriz(tipoMatriz M1, int NLin1, int NCol1, tipoMatriz M2, int NLin2, int NCol2, tipoMatriz MR, int *NLinr, int *NColr)
{
    ...
    *NColr=NCol2;
    ...
    for (l = 0; l < NLin1; l++)
    {
        for (c = 0; c < NCol2; c++)
        {
            for (l1 = 0; l1 < NLin2; l1++)
            {
                aux[l][c]+=(M1[l][l1]*M2[l1][c]);
            }
            MR[l][c]=aux[l][c];
            aux[l][c]=0;
        }
...
{   
    ...
    leiaDimensao(&NLin, &NCol);
    leiaMatriz(M, NLin, NCol);
    mostraMatriz(M, NLin, NCol);
    printf("\nSomatoria = %d", somaMatriz(M, NLin, NCol));
